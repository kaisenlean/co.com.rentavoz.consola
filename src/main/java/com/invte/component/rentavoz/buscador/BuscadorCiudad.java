...
public abstract class BuscadorCiudad extends Buscador<Ciudad> {
    ...
    @Override
        ...
        List<Ciudad> result = getFacade().findByCriterio(getCriterio(),getDepartamento());

        getItems().clear();

        for (Ciudad centrope : result) {
            getItems().add(new BuscadorItem(centrope.getIdCiudad()+"", centrope.getCiuNombre(), centrope));
        }
        return null;
    }
